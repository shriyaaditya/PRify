...
def mock_review_context():
    ...
    symbols = [
        ...
        Symbol(
            name="test_create_user_happy_path",
            kind="function",
...
def test_testing_review_result_validation():
    ...
    assert (
        result.findings[0].suggested_test
        == "test_create_user_invalid_email_raises_value_error"
    )
...
def test_context_formatter_correlation_and_ordering(mock_review_context):
    ...
    formatted = TestingContextFormatter.format_for_testing(
...
def test_positive_case_missing_test_for_new_validation_logic(mock_review_context):
    ...
    mock_llm.generate.return_value = LLMResponse(
        content=llm_payload, token_usage={"total_tokens": 180}, model_name="gpt-4o"
    )
...
def test_negative_case_internal_refactor_with_existing_test_coverage(
    mock_review_context,
):
    ...
    mock_llm.generate.return_value = LLMResponse(
        content=llm_payload, token_usage={"total_tokens": 90}, model_name="gpt-4o"
    )
...
def test_insufficient_context_case(mock_review_context):
    ...
    mock_llm.generate.return_value = LLMResponse(
        content=llm_payload, token_usage={"total_tokens": 80}, model_name="gpt-4o"
    )

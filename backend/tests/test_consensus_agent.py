...
def test_duplicate_findings_consolidated_and_source_agents_preserved(
    mock_review_context,
):
    ...
    mock_llm.generate.return_value = LLMResponse(
        content=llm_payload, token_usage={"total_tokens": 160}, model_name="gpt-4o"
    )
    ...
    assert len(res.findings) == 1
    assert set(res.findings[0].source_agents) == {
        "ArchitectureAgent",
        "PerformanceAgent",
    }
...
def test_same_location_different_concerns_remain_separate(mock_review_context):
    ...
    mock_llm.generate.return_value = LLMResponse(
        content=llm_payload, token_usage={"total_tokens": 200}, model_name="gpt-4o"
    )

...
async def publish_review(
    ...
    if not state.repository or not state.pull_request:
        errors.append(
            "Repository or Pull Request context missing in state. Cannot publish review."
        )
    ...
    if not installation_id:
        errors.append(
            "Installation ID missing in state. Cannot authenticate with GitHub API."
        )
        return {"errors": errors, "logs": logs}
    ...
    if result.get("published"):
        ...
        return {"published_review_id": github_review_id, "logs": logs}
    elif result.get("skipped"):
        logs.append(f"Review publishing skipped: {result.get('reason')}")
        return {"published_review_id": result.get("github_review_id"), "logs": logs}
    else:

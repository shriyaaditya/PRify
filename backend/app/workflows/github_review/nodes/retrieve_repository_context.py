...
async def retrieve_repository_context(
    ...
    test_query = (
        f"test {' '.join(changed_names)}"
        if changed_names
        else "test unit integration specification"
    )

    try:
        results = await retriever.retrieve(
            query=query, repo_fullname=state.repository.full_name, limit=10
        )

        test_results = await retriever.retrieve(
            query=test_query, repo_fullname=state.repository.full_name, limit=10
        )
        ...
        logger.info(
            f"Retrieved {len(retrieved_context)} context chunks for {state.repository.full_name}"
        )

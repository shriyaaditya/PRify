...
async def finish(state: GitHubReviewState, config: RunnableConfig) -> Dict[str, Any]:
    ...
    if state.errors:
        ...
        pr_number = state.pull_request.github_pr_number if state.pull_request else "Unknown"
        repo_name = state.repository.full_name if state.repository else "Unknown"
        logger.info(f"GitHub Review Workflow completed successfully for PR #{pr_number} in {repo_name}")

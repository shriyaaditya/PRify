...
class ArchitectureAgent(BaseReviewAgent):
    ...
    A pure reasoning component that analyzes pull requests for architectural
    violations using tree-sitter symbols and RAG docs. Operates strictly on
    the immutable ReviewContext without making direct external calls.
    """
    ...
    def __init__(
        ...
        self.min_confidence_threshold = float(
            os.getenv("ARCHITECTURE_CONFIDENCE_THRESHOLD", min_confidence_threshold)
        )
    ...
    async def review(self, context: ReviewContext) -> AgentResult:
        logger.info(
            "ArchitectureAgent starting review "
        ...
        logger.info(
            f"Context formatted with budgets. Total Size: {context_size} characters."
        )
        ...
        try:
            ...
            return AgentResult(
                agent_name=self.name, summary="Failed to generate system prompt."
            )

        prompt_size = len(system_prompt)
        logger.info(
            f"System prompt rendered. Total Prompt Size: {prompt_size} characters."
        )
        ...
        messages = [
            ...
            LLMMessage(
                role="user",
                content=(
                    "Please analyze the architectural health and return JSON "
                    "matching the ArchitectureReviewResult schema."
                ),
            ),
        ]
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.info(
                    f"LLM response received. Token Usage: {response.token_usage}"
                )

                # 4. Validate output
                arch_result = ArchitectureReviewResult.model_validate_json(
                    response.content
                )
                logger.info(
                    "Validation successful. "
                    f"Found {len(arch_result.findings)} raw findings."
                )
                ...
                for finding in arch_result.findings:
                    if finding.confidence < self.min_confidence_threshold:
                        logger.info(
                            f"Filtering out finding '{finding.title}' due to low "
                    ...
                    evidence_str = (
                        f"Code Evidence: {finding.code_evidence}\n"
                        f"Docs Evidence: {finding.docs_evidence}"
                    )
                ...
                logger.info(
                    "Final AgentResult constructed with "
                    f"{len(generic_findings)} high-confidence findings."
                )
                return AgentResult(
                ...
                if attempt == max_retries:
                    logger.error(
                        ...
                        "Returning empty AgentResult with error."
                    )
                    return AgentResult(
                        agent_name=self.name,
                        summary="LLM returned malformed JSON that failed validation.",
                    )
                else:
                    messages.append(
                    ...
                    error_msg = (
                        f"Your previous response failed validation with error: {ve}. "
                        "Please fix the JSON output structure."
                    )
                    messages.append(LLMMessage(role="user", content=error_msg))

            except Exception as e:
                logger.error(
                    f"Unexpected error during ArchitectureAgent execution: {e}"
                )
                return AgentResult(
                    agent_name=self.name, summary=f"Execution failed: {str(e)}"
                )

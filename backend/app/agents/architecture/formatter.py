...
class ContextFormatter:
    ...
    Preserves exact information sources and respects configurable context budgets
    ...
    def format_for_architecture(
        ...
        Labeling each source explicitly
        (GitHub Diff, Tree-sitter AST, Qdrant Vector DB).
        """
        formatted_parts = []
        ...
        for cf in context.changed_files[:max_files]:
            ...
            if hasattr(cf, "patch") and cf.patch:
                patch = (
                    cf.patch[:2000] + "\n...[truncated]"
                    if len(cf.patch) > 2000
                    else cf.patch
                )
                formatted_parts.append(f"Patch:\n```\n{patch}\n```")

        if len(context.changed_files) > max_files:
            omitted = len(context.changed_files) - max_files
            formatted_parts.append(
                f"... and {omitted} more changed files omitted due to context budget."
            )
        ...
        if context.symbol_tables:
            ...
            for symbol in context.symbol_tables[:max_symbols]:
                formatted_parts.append(
                    f"- [{symbol.kind}] {symbol.name} (File: {symbol.file_path})"
                )
            if len(context.symbol_tables) > max_symbols:
                omitted_syms = len(context.symbol_tables) - max_symbols
                formatted_parts.append(
                    f"... and {omitted_syms} more symbols omitted "

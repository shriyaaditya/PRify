...
class ArchitectureFinding(BaseModel):
    ...
    reason: str = Field(
        description="Detailed explanation of the architectural violation."
    )
    impact: str = Field(
        description="The potential impact of this violation on the system."
    )
    recommendation: str = Field(
        description="Actionable recommendation to fix the violation."
    )
    code_evidence: str = Field(
        description=(
            "Specific evidence from the changed code (e.g., file paths, "
            "class names, line numbers if available)."
        )
    )
    docs_evidence: str = Field(
        description=(
            "Specific evidence cited from the retrieved repository "
            "documentation (e.g., Qdrant snippets or architecture docs)."
        )
    )
    ...
    suggested_fix: Optional[str] = Field(
        default=None, description="Code snippet demonstrating the suggested fix."
    )
...
class ArchitectureReviewResult(BaseModel):
    ...
    findings: List[ArchitectureFinding] = Field(
        description="List of architectural findings."
    )

...
class PerformanceContextFormatter:
    ...
    def format_for_performance(
        ...
        sorted_files = sorted(
            ...
            key=lambda cf: 0 if cls.is_performance_relevant(cf.filename, getattr(cf, 'patch', '') or '') else 1
        )
        ...
        if context.retrieved_context:
            ...
            for doc in context.retrieved_context:
                if isinstance(doc, dict):
                    ...
                    source = doc.get('metadata', {}).get('source', 'Performance Guideline')
                    docs_text += f"[Document: {source}]\n{content}\n\n"
                elif hasattr(doc, 'page_content'):
                    source = getattr(doc, 'metadata', {}).get('source', 'Performance Guideline')
                    docs_text += f"[Document: {source}]\n{doc.page_content}\n\n"
                else:
                    docs_text += f"{str(doc)}\n\n"

...
class PerformanceAgent(BaseReviewAgent):
    ...
    async def review(self, context: ReviewContext) -> AgentResult:
        ...
        logger.info(
            f"Performance context formatted. Total Size: {context_size} characters."
        )
        ...
        logger.info(
            f"Performance system prompt rendered. Prompt Size: {prompt_size} characters."
        )
        ...
        messages = [
            ...
            LLMMessage(
                role="user",
                content="Please perform the performance and scalability review and return JSON matching the PerformanceReviewResult schema.",
            ),
        ]
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.info(
                    f"LLM performance response received. Tokens used: {tokens_used}"
                )

                # 4. Validate output
                perf_result = PerformanceReviewResult.model_validate_json(
                    response.content
                )
                raw_findings_count = len(perf_result.findings)
                logger.info(
                    f"Performance output validation successful. Raw findings: {raw_findings_count}"
                )
                ...
                logger.warning(
                    f"Performance JSON validation failed on attempt {attempt + 1}: {ve}"
                )
                if attempt == max_retries:
                    logger.error(
                        "Max retries reached for Performance Agent JSON validation. Returning error result."
                    )
                    return AgentResult(
                        agent_name=self.name,
                ...
                return AgentResult(
                    agent_name=self.name,
                    summary=f"Performance execution failed: {str(e)}",
                )

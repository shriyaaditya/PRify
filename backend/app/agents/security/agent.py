...
class SecurityAgent(BaseReviewAgent):
    ...
    async def review(self, context: ReviewContext) -> AgentResult:
        ...
        logger.info(
            f"Security context formatted. Total Size: {context_size} characters."
        )
        ...
        logger.info(
            f"Security system prompt rendered. Prompt Size: {prompt_size} characters."
        )
        ...
        messages = [
            ...
            LLMMessage(
                role="user",
                content="Please perform the security review and return JSON matching the SecurityReviewResult schema.",
            ),
        ]
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.info(
                    f"LLM security response received. Tokens used: {tokens_used}"
                )
                ...
                logger.info(
                    f"Security output validation successful. Raw findings: {raw_findings_count}"
                )
                ...
                for finding in sec_result.findings:
                    ...
                    evidence_parts = [
                        f"Code: {finding.code_evidence}",
                        f"Docs: {finding.docs_evidence}",
                    ]
                ...
                logger.warning(
                    f"Security JSON validation failed on attempt {attempt + 1}: {ve}"
                )
                if attempt == max_retries:
                    logger.error(
                        "Max retries reached for Security Agent JSON validation. Returning error result."
                    )
                    return AgentResult(
                        agent_name=self.name,
                ...
                return AgentResult(
                    agent_name=self.name, summary=f"Security execution failed: {str(e)}"
                )

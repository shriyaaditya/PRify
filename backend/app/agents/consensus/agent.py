...
class ConsensusAgent:
    ...
    async def consolidate(
        ...
        if input_findings_count == 0:
            ...
            logger.info(f"ConsensusAgent completed in {duration_ms:.2f}ms. No specialist findings to consolidate.")
            return ConsensusReviewResult(
                summary="Consensus review complete. No issues were flagged by specialist agents.",
                findings=[]
            )
        ...
        logger.info(f"Consensus context formatted. Total Size: {context_size} characters.")
        ...
        logger.info(f"Consensus system prompt rendered. Prompt Size: {prompt_size} characters.")

        messages = [
            LLMMessage(role="system", content=system_prompt),
            LLMMessage(role="user", content="Please consolidate the specialist findings and return JSON matching the ConsensusReviewResult schema.")
        ]
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.info(f"LLM consensus response received. Tokens used: {tokens_used}")

                # 4. Validate output
                consensus_result = ConsensusReviewResult.model_validate_json(response.content)
                ...
                suppressed_findings_count = max(0, input_findings_count - accounted_input_findings)
                ...
                logger.warning(f"Consensus JSON validation failed on attempt {attempt + 1}: {ve}")
                if attempt == max_retries:
                    logger.error("Max retries reached for Consensus Agent JSON validation. Returning empty result.")
                    return ConsensusReviewResult(
                        summary="LLM returned malformed JSON that failed ConsensusReviewResult validation.",
                        findings=[]
                    )
                else:
                    retry_attempts += 1
                ...
                return ConsensusReviewResult(
                    summary=f"Consensus execution failed: {str(e)}",
                    findings=[]
                )

        return ConsensusReviewResult(
            summary="Consensus review failed.",
            findings=[]
        )

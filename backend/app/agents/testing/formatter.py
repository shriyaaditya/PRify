...
class TestingContextFormatter:
    ...
    def format_for_testing(
        ...
        for cf in sorted_files[:max_files]:
            ...
            file_type_label = "Test File" if cls.is_test_file(cf.filename) else "Production Code"
            formatted_parts.append(f"--- File: {cf.filename} (Status: {status_str}, Type: {file_type_label}) ---")
        ...
        if context.symbol_tables:
            ...
            for symbol in context.symbol_tables[:max_symbols]:
                file_path_str = getattr(symbol, 'file_path', getattr(symbol, 'filepath', 'N/A'))
                formatted_parts.append(f"- [{symbol.kind}] {symbol.name} (File: {file_path_str})")
        ...
        if context.retrieved_context:
            ...
            for doc in context.retrieved_context:
                if isinstance(doc, dict):
                    ...
                    source = doc.get('source', doc.get('metadata', {}).get('source', doc.get('metadata', {}).get('path', 'Repository Context')))
                    doc_type = doc.get('document_type', doc.get('metadata', {}).get('document_type', 'OTHER'))
                    docs_text += f"[Document: {source} | Type: {doc_type}]\n{content}\n\n"
                elif hasattr(doc, 'page_content') or hasattr(doc, 'content'):
                    content = getattr(doc, 'content', getattr(doc, 'page_content', str(doc)))
                    meta = getattr(doc, 'metadata', {})
                    source = getattr(meta, 'source', getattr(meta, 'path', 'Repository Context')) if hasattr(meta, 'source') or hasattr(meta, 'path') else meta.get('source', meta.get('path', 'Repository Context')) if isinstance(meta, dict) else 'Repository Context'
                    doc_type = getattr(meta, 'document_type', 'OTHER') if hasattr(meta, 'document_type') else meta.get('document_type', 'OTHER') if isinstance(meta, dict) else 'OTHER'
                    docs_text += f"[Document: {source} | Type: {doc_type}]\n{content}\n\n"
                else:
                    docs_text += f"{str(doc)}\n\n"

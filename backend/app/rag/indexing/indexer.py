...
class Indexer:
    ...
    async def index_repository(self, repository_id: uuid.UUID, repo_fullname: str, branch: str = "main", collection_name: str = "repository_docs"):
        """
        ...
        loader = RepositoryLoader(gh_client=self.gh_client, repo_fullname=repo_fullname, branch=branch)
        ...
        result = await self.session.execute(
            select(IndexedDocument).where(IndexedDocument.repository_id == repository_id)
        )
        existing_records = result.scalars().all()
        existing_checksums = {doc.document_name: doc.checksum for doc in existing_records}
        ...
        # Process and embed
        chunks = self.chunker.chunk_documents(repository=repo_fullname, docs=docs_to_index)
            ...
            for doc in docs_to_index:
                ...
                if doc.path in existing_docs_dict:
                    ...
                    new_indexed_doc = IndexedDocument(
                        ...
                        qdrant_point_id=str(uuid.uuid4()), # We generate a UUID for the record, though chunks have their own IDs
                        checksum=doc.checksum
                    )
                    self.session.add(new_indexed_doc)
            
            await self.session.commit()
            logger.info(f"Successfully indexed {len(docs_to_index)} documents for {repo_fullname}.")

...
from app.schemas.review import ReviewCreate, ReviewFindingCreate
...
class ReviewPublisherService:
    ...
    async def publish_review(
        ...
        if db and pr_uuid and head_sha:
            ...
            if (
                existing_review
                and existing_review.status == ReviewStatus.COMPLETED
                and existing_review.github_review_id
            ):
                logger.info(
                    f"Review already published for PR {pr_number} commit {head_sha} (GitHub Review ID: {existing_review.github_review_id}). Skipping duplicate publishing."
                )
                return {
                    "published": False,
                    "skipped": True,
        ...
        payload = GitHubReviewFormatter.format_github_review_payload(
            consensus_result=consensus_result, changed_files=changed_files
        )

        logger.info(
            f"Publishing PR review to {repo_owner}/{repo_name}#{pr_number} for commit {effective_sha} with {len(payload.get('comments', []))} inline comments."
        )
        ...
        try:
            ...
            if response.status_code in (200, 201):
                ...
                logger.info(
                    f"Successfully published GitHub review {github_review_id} for PR #{pr_number}."
                )
                ...
                return {"published": False, "error": err_msg}

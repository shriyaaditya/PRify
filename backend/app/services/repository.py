...
class RepositoryService:
    async def get_repository(
        self, db: AsyncSession, repo_id: uuid.UUID
    ) -> Optional[Repository]:
        return await repo_module.repository.get(db, id=repo_id)

    async def get_by_github_repo_id(
        self, db: AsyncSession, github_repo_id: str
    ) -> Optional[Repository]:
        return await repo_module.repository.get_by_github_repo_id(
            db, github_repo_id=github_repo_id
        )
    ...
    async def create_repository(
        self, db: AsyncSession, repo_in: RepositoryCreate
    ) -> Repository:
